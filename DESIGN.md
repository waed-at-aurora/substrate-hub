...
- **Composition:** no masthead, rail, or document frame. A `100svh` Paper stage carries the ruled dateline, the flexible wordmark field, and an unruled body band so the grid remains visually continuous. The body presents positioning, concise composite-layer copy, `Enter the hub` / `Install Substrate` / `Explore patterns`, then the real CLI install command.
...
- **Composition:** a `760svh` runway (`680svh` at ≤52rem) holds one sticky `100svh` drafting plate. A real-time Three.js pyramid occupies the field while four compact direct-address controls and one progress rule stay fixed. The tour is meaningful bottom-to-top: Tokens → Primitives → Composites → Products.
...
- **Scroll:** Anime.js's Three adapter animates the actual `Object3D` rotation, camera, look target, tier lift, and tier scale. Its Scroll Observer scrubs a single 10.7-second authored timeline: spin → zoom in → explain → zoom out, repeated once per tier, then one final zoom-out into the overview invitation.
- **Interaction:** pointer position adds a bounded orbit offset. Raycasting brightens the tier under the cursor; selecting it seeks the matching scroll chapter. The four ruled layer buttons provide the same direct address for keyboard and touch users.
- **Ending:** the final chapter recedes the pyramid and reveals “Ready to explore Substrate?” with the surface's yellow `Open the overview` action.
- **Reduced motion:** sticky choreography is removed. The complete static Three.js pyramid, all four concise explanations, and the final overview action render as a normal document sequence; progress resolves to 100%. If WebGL cannot initialize, a semantic four-tier CSS pyramid remains visible.
