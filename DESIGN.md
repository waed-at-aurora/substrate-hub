...
- **Continuous plate:** `.landing-home` owns one uninterrupted 8-column × 4-row Rule Soft grid behind both components. System Construction overlaps the hero by `18svh` on desktop and `10svh` on mobile, preserving its effective 760/680svh runway while letting the real Three.js model enter before the hero exits. The single shooting-line canvas is bounded to the hero plus overlap and shares the model/tether anchor; there is no separate transition section, added runway, or intercepted gesture.
- **Occlusion:** a bounded Paper radial wash protects the hero copy without drawing a rectangular panel. Construction copy remains at zero opacity through the overlap and reveals only as the sticky model takes over; the shooting-line bloom, tether convergence, and pyramid top align into one handoff point.
