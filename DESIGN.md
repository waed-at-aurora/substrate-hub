...
The root route is one continuous portal-to-system scene. Its first viewport contains the dateline, monumental SUBSTRATE wordmark, approved positioning, four task-first actions led by yellow `Open overview`, and the install line. The moving lamp, revealed fine grid, ghost parallax, and per-letter weight response remain the landing's authored identity. A restrained half-circle field of shooting assembly lines originates at the landing's lower-right corner and overlaps the real pyramid entering beneath the hero.
...
- **Composition:** no masthead, rail, or document frame. A `100svh` Paper stage carries the ruled dateline, the flexible wordmark field, and an unruled body band so the grid remains visually continuous. The body positions Substrate as “The foundational medium from which experiences emerge.” and “An agent-first, modern design system.” before `Open overview` / `Browse components` / `Install Substrate` / `Explore patterns` and the real CLI install command. The `/overview` stage repeats the same positioning verbatim.
