...
The root route is one continuous portal-to-system scene. Its first viewport contains the dateline, monumental SUBSTRATE wordmark, approved positioning, a compact agent-ready setup prompt, and a centered animated scroll cue. The moving lamp, revealed fine grid, ghost parallax, and per-letter weight response remain the landing's authored identity. A restrained half-circle field of shooting assembly lines originates at the landing's lower-right corner and overlaps the real pyramid entering beneath the hero.

**System Construction** begins before the first viewport has fully left: the landing and the full-viewport, scroll-scrubbed 3D tour overlap on one shared fine drafting plate. The fixed star source remains spatially stable while the hero copy stays legible; construction copy waits until the landing body clears. The model then rotates through each tier, names that tier in a few words, returns to the complete architecture, and ends with a direct handoff into `/overview`.
...
### Named Rules
**The Yellow Law.** In hub chrome, #ffcc00 may appear only on the one primary action of a surface, on the `new` signal, and on the contents rail's active-section number chip. A dark yellow **wash** (22% Yellow into Paper) additionally grounds the pyramid's highest-leverage UX layer. Content rendered by `@aurora-ui/substrate` (e.g. the `eos` Button variant) is content, not chrome — its yellow is exempt.
...
**The Code Panel Rule.** CopyBlock, InstallLine, and the playground's import-snippet panel share one restrained neutral chrome — a 3% Ink wash into Paper Raised with a 20% Ink Rule border — not the Yellow Law's accent (revised 2026-09-10: these panels read as implementation material, not as chrome needing the primary-action color). Code *content* inside them, though, is exempt from the hub's accent laws exactly like a live `@aurora-ui/substrate` render is — it is real source, not chrome — so it carries a conventional editor syntax palette (`--syn-kw` blue, `--syn-type` teal, `--syn-fn` gold, `--syn-attr` light blue, `--syn-str` orange) instead of the hub's zinc/yellow/cyan tokens (revised 2026-09-10, reference: a VS Code Dark+ excerpt). Multi-line panels number every line in a dim, non-selectable Muted gutter, tabular-nums, flush right.
...
The root `/` route deliberately sits outside the document shell. Its landing stage is exactly one small viewport tall (`100svh`) on larger screens and uses a vertical composition: ruled dateline, flexible centered wordmark field, then a seamless bottom body band. The containing `.landing-home` clips page edges while the bounded shooting-star layer is allowed to cross the hero boundary into the construction overlap. The band holds the pitch at left, the live lamp-coordinate readout at right, and a centered “See why substrate matters” cue without drawing a divider through the illuminated grid. The cue's drafting stroke repeatedly traces downward to make the scroll relationship explicit; it pauses offscreen or while the document is hidden. The stage has no direct navigation actions.

Inside `/overview` and the numbered hub routes, the document frame is a two-column grid — a 15.5rem sticky contents rail plus a fluid page column — capped at 90rem and padded by a fluid gutter (`clamp(1.25rem, 3.5vw, 3rem)`). A sticky, blur-backed masthead (paper at 88% opacity, backdrop blur 10px, hairline bottom) sits above; the rail sticks at 3.4rem below it and carries the seven numbered sections (01–07) with a `kbd` hint — keys 1–7 navigate directly.

Responsive: at 64rem the persona navigator, playground, pyramid, and resource columns collapse to one column. At 52rem the hub frame goes single-column, the rail flattens into a horizontal wrap (borders and hint dropped), the masthead subtitle and overview-stage ghost disappear, the persona selector becomes a complete 2 × 2 role index above a stacked value narrative, and the roadmap transmission shifts from a 16:9 frame to a taller mobile monitor with a stacked timecode. At the same breakpoint the root landing becomes scroll-safe (`height: auto; min-height: 100svh`), hides its ghost and coordinate readout, gives the wordmark field a bounded 18–25rem / 43svh height, stacks the dateline, and centers the continuation cue beneath the pitch. The compact prompt fills the available width while its one-line preview remains clipped with an ellipsis.


The `/roadmap` route is intentionally an unavailable-state surface until its planning source is ready. A single top-aligned, full-width analogue transmission frame replaces provisional priorities: low-resolution canvas grain, CRT scanlines, intermittent head-switch noise, restrained cyan/yellow chroma slips, tracking telemetry, and the oversized `COMING SOON` message. A locally served rubber-duck GLB is rendered behind the message through a one-bit Bayer pass adapted from Canvas UI's Dithered Object treatment; its yaw and pitch ease toward the cursor across the entire transmission frame, then settle to center when the pointer leaves. The semantic message remains ordinary HTML above both decorative canvases. Grain is capped at 12fps, and both render loops stop offscreen or while the document is hidden; reduced motion holds the duck and grain still and removes every tracking or chroma loop.
...
- **Composition:** no masthead, rail, or document frame. A `100svh` Paper stage carries the ruled dateline, the flexible wordmark field, and an unruled body band so the grid remains visually continuous. The body positions Substrate as “The foundational medium from which experiences emerge.” and “An agent-first, modern design system.” above an `Agent-ready setup` prompt that shows only a one-line preview while copying the complete guided setup instruction. A horizontally centered continuation cue reads “See why substrate matters”; the four direct navigation actions are intentionally absent. Its arrow is a drawn drafting stroke that traces downward on a restrained loop, pauses whenever the stage or document is hidden, and resolves to a static complete mark under reduced motion. The `/overview` stage repeats the positioning verbatim.
...
- **Shooting assembly lines:** one bounded canvas spans the hero and overlap. It draws 28 deterministic radial guides with 15 staggered pulses from an origin anchored at the first component's lower-right corner. Because the canvas extends 18svh past the hero height, the visible burst reads as a full left-facing half circle that overlaps System Construction; the layer carries no pointer parallax and no scroll-scrubbed movement of its own — it holds its corner and travels with the plate. Device-pixel ratio is capped at 2.
- **Touch:** when hover is unavailable, and before the first real pointer movement on hover-capable devices, the lamp follows a slow sinusoidal autopilot. Direct layer controls remain the model's explicit mobile interaction.
- **Reduced motion:** the pointer animation loop never starts and the shooting lines render one quiet still frame. The CSS defaults hold a static lamp at 30% / 24%, the wordmark stays fully legible at 'wght' 760 / 'opsz' 96, the ghost does not parallax, the overlap is removed, and System Construction becomes a static document sequence.
...
### Portal-to-construction handoff
- **Continuous plate:** `.landing-home` owns one uninterrupted 2.6rem square Rule Soft grid behind both components. System Construction overlaps the hero by `18svh` on desktop and `10svh` on mobile, preserving its effective 760/680svh runway while letting the real Three.js model enter before the hero exits. The single shooting-line canvas is anchored to the first component's lower-right corner and crosses into the overlap; there is no separate transition section, added runway, or intercepted gesture.
- **Occlusion:** a bounded Paper radial wash protects the hero copy without drawing a rectangular panel. Construction copy remains at zero opacity through the overlap and reveals only after the landing body clears; the shooting-line half circle sits behind both components rather than riding the scroll choreography. A separate Paper occlusion field behind the model blocks the page grid and stays at full strength through the final invitation while the geometry recedes.

### System Construction (landing continuation)
- **Composition:** a `778svh` runway overlapping the hero by `18svh` (mobile: `690svh` / `10svh`) yields the established effective 760/680svh scroll tour and holds one sticky `100svh` drafting plate. On desktop, the Three.js canvas is full-bleed across the plate and uses an off-axis camera composition that places the pyramid's center at 68% of the viewport width—the middle-right field—while allowing it to pass behind the framing copy and narrative; the four compact direct-address controls remain a ruled foreground layer above it. At ≤52rem the established centered mobile composition remains unchanged. The tour is meaningful bottom-to-top: Tokens establish shared decisions; Primitives are shareable components built once and reused instead of rebuilt product by product; Composites are complete, production-ready components; Products present one unified EOS experience across Chronos, Amun, Origin, Solaris, and Lumus.
...
### Persona value navigator (`/overview`)
- **Purpose:** translates the same system into three outcome-led views without duplicating the page: Dev teams get setup, discovery, composition, and workflow links; Product teams get pattern discovery, real-part prototyping, and platform guidance; Commercial gets demo-ready capability, roadmap visibility, and client-facing stories.
- **Interaction:** an accessible three-tab selector uses automatic activation, Arrow Left/Right, Home, and End keyboard navigation with one labelled tabpanel. Active selection follows the Inversion Rule—Ink on Paper—rather than adding an accent. Each view pairs one large outcome statement with a four-step value path, three concise benefits, and three direct next actions.
- **Implementation:** React owns the semantic tab state; the already-installed Motion library animates only the changing panel with a 420ms clip/blur/translation reveal. The design-system components barrel is intentionally not imported here because its charting side effects are unsafe during static prerendering. `prefers-reduced-motion` skips the panel animation.
...
- **CopyBlock:** Paper Raised with the shared 3% Ink wash, a 20% Ink Rule border, and a 3px radius (The Code Panel Rule). Its mono uppercase label is a 40% Ink mix into Muted; 0.82rem code carries real editor-style syntax highlighting via a narrow per-language tokenizer (`bash` / `json` / `tsx`) — `--syn-kw` for keywords, `--syn-type` for JSX tags, `--syn-fn` for the leading shell command, `--syn-attr` for attributes and JSON keys, `--syn-str` for strings — each multi-line panel numbered in a dim Muted gutter; the absolute `copy` button stays Paper with a Rule border. The button is mono uppercase 0.7rem; on success it turns OK Green and reads `copied` for 1.6s. Prompt blocks skip tokenizing and the gutter, and wrap in Archivo, because they are instructions to read and paste rather than source code. The root landing uses the compact prompt variant: it renders a one-line ellipsized preview while the copy action writes the complete shared setup prompt.
- **InstallLine:** the compact single-command variant — inline flex on the same 3% Ink-washed Paper Raised panel with a 20% Ink Rule border (The Code Panel Rule), the same `bash` syntax highlighting (no gutter — one line needs no line number), a non-selectable muted `$` prompt, and a fixed-width inline copy button. Success reads `copied` for 1.6s and is announced politely; when the Clipboard API is unavailable, the command is selected in place and the button reads `selected` so manual copy remains obvious.
...
- Specimen stages use Paper Raised and the established drafting grid; the stage's single grid track is stretched (`minmax(0, 1fr)`) so a specimen's own width rule resolves against the real box at every breakpoint instead of the item's shrink-to-fit size. The hub frames but does not visually rewrite package components. Search empty state offers one explicit gallery reset; no source rows or code snippets appear.
...
- **Composite-first (added 2026-09-10):** the gallery leads with composites, not primitives — `InvestmentCaseSelectionCard` (hero), `TimelineScrubber`, `ChartHC`, `DataTable`, and `ChartToolbar` (wide), then `TimeRangeSelector`, `SurfaceToolbar`, `SurfacePanel`, and `ModeActivatorToggleGroup` (standard) all render live and controlled, matching Product Principle 2 (composites are the product). `ChartHC` is the one specimen from the package's separate `@aurora-ui/substrate/charts` export (Highcharts is bundled into that subpath's build, so the hub declares it as a direct dependency purely for type resolution — never imported at runtime outside the DS bundle). A data-dense specimen (`DataTable`, `ChartHC`, `TimelineScrubber`, `ChartToolbar`/`SurfaceToolbar`) fills its stage edge-to-edge like it would in a real product surface, rather than shrink-wrapping like the compact card/tab/slider specimens; `DataTable` inherits the package's own horizontal scroll (`data-slot="table-container"`) when a narrower box genuinely can't hold every column, the same `.table-scroll` treatment already used for the overview coverage table.
...
### Motion
All micro-transitions run 120–160ms on `--ease-out` (`cubic-bezier(0.16, 1, 0.3, 1)`). Motion is sparse and role-bound: identity uses the 1100ms variable-weight settle; the root stage combines direct lamp, per-letter response, and corner-anchored shooting assembly lines; the real pyramid rises through the overlapping drafting plate; System Construction then uses proportional native scrolling with Anime.js `0.35` eased synchronization and its Three adapter for the four-chapter camera tour; `/overview` retains its 18s alternating ghost drift, Motion-driven stage exit, Anime.js caption/list assembly, and one 420ms Motion reveal when the visitor changes persona; the playground uses a 700ms cyan recalc flash; the contents rail answers navigation in ~200ms; and the pyramid uses its measured 380/240ms close, 120ms exchange, and 550/450ms open.

**The Authored Motion Rule.** New motion must justify itself as identity, atmosphere, liveness, navigation feedback, or spatial continuity. Under `prefers-reduced-motion: reduce`, the root shooting lines resolve to one still frame, the stage does not create its pointer animation loop, the portal overlap is removed, System Construction becomes a static explorable document, the overview scroll choreography detaches and restores static content, all keyframes are disabled, smooth scroll turns off, and every transition collapses to 0.01ms.
...
- **Do** keep the lamp, pointer/touch response, per-letter weight behavior, and entering model together on `/`; hand visitors into the document at `/overview`.
...
- **Don't** generalize the root landing's pointer lamp, overlapping model, or per-letter response into the numbered hub pages.
