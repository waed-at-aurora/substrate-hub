...
- **Ending:** the final chapter recedes the pyramid and reveals “Ready to explore Substrate?” with the surface's yellow `Open the overview` action. Activating that action drives the four LEGO tiers off-screen in distinct directions over 820ms while the 3D grid and interface dissolve into pure black; a 220ms cleared-screen hold completes the exit before navigation to `/overview`.
