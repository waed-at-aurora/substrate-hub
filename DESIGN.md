...
The root route is one continuous portal-to-system scene. Its first viewport contains the dateline, monumental SUBSTRATE wordmark, positioning, four task-first actions led by yellow `Open overview`, and the install line. The moving lamp, revealed fine grid, ghost parallax, and per-letter weight response remain the landing's authored identity. A restrained field of shooting assembly lines converges above the real pyramid entering beneath the hero.
...
- **Shooting assembly lines:** one bounded canvas spans the hero and overlap. It draws 22 deterministic radial guides with 15 staggered pulses each from the model anchor—68% viewport width on desktop and the centered mobile composition—so the visible “stars” become construction geometry as the pyramid arrives. Drawing pauses when the layer or document is hidden; device-pixel ratio is capped at 2.
...
- **Continuous plate:** `.landing-home` owns one uninterrupted 2.6rem square Rule Soft grid behind both components. System Construction overlaps the hero by `18svh` on desktop and `10svh` on mobile, preserving its effective 760/680svh runway while letting the real Three.js model enter before the hero exits. The single shooting-line canvas is bounded to the hero plus overlap and shares the model anchor; there is no separate transition section, added runway, or intercepted gesture.
- **Occlusion:** a bounded Paper radial wash protects the hero copy without drawing a rectangular panel. Construction copy remains at zero opacity through the overlap and reveals only after the landing body clears; the shooting-line bloom and pyramid top align into one handoff point.
...
### Motion
All micro-transitions run 120–160ms on `--ease-out` (`cubic-bezier(0.16, 1, 0.3, 1)`). Motion is sparse and role-bound: identity uses the 1100ms variable-weight settle; the root stage combines direct lamp, per-letter response, and model-anchored shooting assembly lines; the real pyramid shares that pointer vector while rising through the overlapping drafting plate; System Construction then uses proportional native scrolling with Anime.js `0.35` eased synchronization and its Three adapter for the four-chapter camera tour; `/overview` retains its 18s alternating ghost drift and adds a Motion-driven stage exit plus Anime.js caption/list assembly; the playground uses a 700ms cyan recalc flash; the contents rail answers navigation in ~200ms; and the pyramid accordion uses a staged 380ms close / 550ms open handoff with a 120ms bridge.
