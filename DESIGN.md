...
- **Purpose:** translates the same system into four outcome-led views without duplicating the page: Dev teams get setup, discovery, composition, and workflow links; Product teams get pattern discovery, real-part prototyping, and platform guidance; Senior stakeholders get speed, reduced duplication, and differentiated client value; Commercial gets demo-ready capability, roadmap visibility, and client-facing stories.
- **Interaction:** an accessible four-tab selector uses automatic activation, Arrow Left/Right, Home, and End keyboard navigation with one labelled tabpanel. Active selection follows the Inversion Rule—Ink on Paper—rather than adding an accent. Each view pairs one large outcome statement with a four-step value path, three concise benefits, and three direct next actions.
