...
The root route is the portal before the document: one full-viewport, pointer-responsive stage containing the dateline, monumental SUBSTRATE wordmark, positioning, three actions led by yellow `Enter the hub`, and the install line. Its moving lamp, projected Aurora symbol, revealed grid, lower-right Star Burst, ghost parallax, and per-letter weight response form one authored invitation into `/overview`; they are a landing-only behavior, not a template for the numbered hub pages.
...
- **Lamp and grid:** two circular radial gradients follow `--lx` / `--ly`; the outer gradient vignettes the edges while the brighter Rule/Ink-mixed core opens onto the Paper ground. A 2.6rem two-axis zinc grid is visible only through a matching circular radial mask centered on the lamp.
...
- **Eclipse Signal (user-selected 2026-08-27):** the official Aurora sun-and-horizon symbol is cropped from the existing Aurora artwork and used as an alpha mask, never redrawn. It sits inside the lamp as a monochrome projected shadow (50% opacity, 0.7px optical blur) and counter-moves 18px × 12px against the pointer so it reads as projection rather than a pasted logo. The full wordmark and decorative Aurora Yellow are deliberately excluded.
...
- **Reduced motion:** the pointer animation loop never starts. The CSS defaults hold a static lamp and Eclipse Signal at 30% / 24%, the Star Burst warms up to one still frame, the wordmark stays fully legible at 'wght' 760 / 'opsz' 96, the ghost does not parallax, and transitions collapse with the global reduced-motion rule.
...
- **Do** keep the lamp, Eclipse Signal, Star Burst, pointer/touch response, and per-letter weight behavior together on `/`; hand visitors into the document at `/overview`.
...
- **Don't** generalize the root landing's pointer lamp, projected Aurora mark, Star Burst canvas, or per-letter response into the numbered hub pages.
