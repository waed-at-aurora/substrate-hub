...
The root route is one continuous portal-to-system scene. Its first viewport contains the dateline, monumental SUBSTRATE wordmark, positioning, four task-first actions led by yellow `Open overview`, and the install line. The moving lamp, revealed fine grid, ghost parallax, and per-letter weight response remain the landing's authored identity. Two drafting tethers and a restrained field of shooting assembly lines converge on the same point above the real pyramid entering beneath the hero.

**System Construction** begins before the first viewport has fully left: the landing and the full-viewport, scroll-scrubbed 3D tour overlap on one shared coarse drafting plate. The star source resolves into the top of the arriving pyramid while the hero copy stays legible; construction copy waits until the model owns the viewport. The model then rotates through each tier, names that tier in a few words, returns to the complete architecture, and ends with a direct handoff into `/overview`.
...
- **Ghost tether:** two 1px drafting guides run from the eased cursor position toward the model's lower-right field. One is solid and one dashed; their opacity rises modestly as the visitor approaches the handoff. The model reads the same global pointer vector before its own canvas becomes interactive, so the cursor moves one spatial system rather than triggering unrelated effects.
...
- **Touch:** when hover is unavailable, and before the first real pointer movement on hover-capable devices, the lamp follows a slow sinusoidal autopilot. Touch hides the drafting tethers; direct layer controls remain the model's explicit mobile interaction.
...
- **Continuous plate:** `.landing-home` owns one uninterrupted 8-column × 4-row Rule Soft grid behind both components. System Construction overlaps the hero by `18svh` on desktop and `10svh` on mobile, preserving its effective 760/680svh runway while letting the real Three.js model enter before the hero exits. The single shooting-line canvas is anchored to the first component's right corner and crosses into the overlap; there is no separate transition section, added runway, or intercepted gesture.
- **Occlusion:** a bounded Paper radial wash protects the hero copy without drawing a rectangular panel. Construction copy remains at zero opacity through the overlap and reveals only as the sticky model takes over; the shooting-line half circle sits behind both components rather than riding the scroll choreography.
...
- **Do** keep the lamp, ghost tether, pointer/touch response, per-letter weight behavior, and entering model together on `/`; hand visitors into the document at `/overview`.
...
- **Don't** generalize the root landing's pointer lamp, ghost tether, overlapping model, or per-letter response into the numbered hub pages.
