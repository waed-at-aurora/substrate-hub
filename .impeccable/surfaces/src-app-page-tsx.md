...
Hero (user-pinned requirements): a ruled `100svh` stage; SUBSTRATE wordmark at viewport-cropping scale with a one-time variable-weight settle and an outline ghost; positioning line "The shared layer behind consistent EOS experiences."; supporting composite-layer copy; four task-first actions led by yellow `Open overview`; and the compact copyable real CLI install command. Model-anchored shooting lines are spatial continuity, not a separate decorative section.
...
Landing continuation (user-redesigned 2026-08-28; unified 2026-09-01; shooting lines reconnected 2026-09-01): the hero and full-viewport Three.js tour share one uninterrupted drafting plate. The tour overlaps the hero so the real pyramid enters before the task actions leave. Two quiet guides tether the cursor-responsive ghost toward the desktop model field, while 22 deterministic shooting-star spokes originate at the same anchor and resolve visually into the pyramid top; mobile recenters that source over its centered model. Construction copy stays hidden until the model takes over, touch removes only the tethers, and reduced motion preserves one quiet star frame while removing the overlap. The tour continues through Tokens → Primitives → Composites → Products with Anime.js scroll choreography, pointer orbit, raycast tier selection, keyboard/touch controls, live counts, and the final overview action.
