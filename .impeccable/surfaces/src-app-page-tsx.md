...
Hero (user-pinned requirements): a ruled `100svh` stage; SUBSTRATE wordmark at viewport-cropping scale with a one-time variable-weight settle and an outline ghost; positioning lines "The foundational medium from which experiences emerge." and "An agent-first, modern design system."; the same positioning repeated on `/overview`; four task-first actions led by yellow `Open overview`; and the compact copyable real CLI install command. Model-anchored shooting lines are spatial continuity, not a separate decorative section.
...
Landing continuation (user-redesigned 2026-08-28; unified 2026-09-01; shooting lines reconnected 2026-09-01; corner-anchored half-circle source 2026-09-02): the hero and full-viewport Three.js tour share one uninterrupted fine drafting plate. The tour overlaps the hero so the real pyramid enters before the task actions leave. Twenty-eight deterministic shooting-star spokes originate from a source anchored at the landing component's lower-right corner and form a full left-facing half circle that overlaps into System Construction; the layer holds that corner without pointer parallax or scroll-scrubbed movement. Construction copy stays hidden until the landing body clears, touch preserves the same fixed source, and reduced motion keeps the hierarchy as a static document. The tour continues through Tokens → Primitives → Composites → Products with Anime.js scroll choreography, pointer orbit, raycast tier selection, keyboard/touch controls, live counts, and the final overview action.
